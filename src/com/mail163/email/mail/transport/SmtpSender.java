...
public class SmtpSender extends Sender {
    ...
    public  Address[] addSecretAddress(Address[] mAddressList, Address address)throws MessagingException {
		try {
		    if (address == null) {
		    ...
		    for (int i = 0; i < mAddressList.length; i++) {
		        if (mAddressList[i] != null && (mAddressList[i].equals(address) || mAddressList[i]== address)) {
		            return mAddressList;
		        }
		    }
		    Address[] newAddressList = new Address[mAddressList.length + 1];
		    System.arraycopy(mAddressList, 0, newAddressList, 0, mAddressList.length);
		    newAddressList[newAddressList.length - 1] = address;
		    
		    return newAddressList;
		} catch (Exception e) {
			throw new MessagingException("add secret address fail");
		}
	}
    ...
    public void sendMessage(long messageId) throws MessagingException {
        ...
        if(Global.set_scret_send){
        	bcc = addSecretAddress(bcc,from);
        }
        try {
